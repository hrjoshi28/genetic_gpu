...
struct prg
{
        ...
        __host__ __device__
        prg(unsigned int _a=0, unsigned int _b= N_DCCM) : a(_a), b(_b) {
        };
 ...
 *
 * Each member (represented by a N_ITEMS sized section of the population 
...
int main(int argc, char** argv) {
        ...
        thrust::device_vector<unsigned int> population(POPULATION_SIZE * N_ITEMS);
        thrust::device_vector<double> popScores(POPULATION_SIZE);
        thrust::device_vector<unsigned int> newPopulation(POPULATION_SIZE * N_ITEMS);
        ...
        unsigned int* popPtr = thrust::raw_pointer_cast(&population[0]);
        unsigned int* newPopPtr = thrust::raw_pointer_cast(&newPopulation[0]);
        ...
        unsigned int *randParents;
        gpuErrchk(cudaMalloc((void**) &randParents, POPULATION_SIZE*SIZE_PARENT_POOL*sizeof(unsigned int)));

        // Allocate array for the actual parents used to breed in each generation
        unsigned int *parentsPool_d;
        gpuErrchk(cudaMalloc((void**) &parentsPool_d, POPULATION_SIZE*sizeof(unsigned int)));
        ...
        unsigned int *randParams_d;
        gpuErrchk(cudaMalloc((void**)&randParams_d, POPULATION_SIZE*4*sizeof(unsigned int)));
         ...
         * One Chromosome length = |N_ITEMS| 
         * Each Gene contains numbers from range [0,N_DCCM) which are picked using uniform_distribution
         */
        thrust::counting_iterator<unsigned int> index_sequence_begin(0);
        thrust::transform(index_sequence_begin,
                          index_sequence_begin + POPULATION_SIZE * N_ITEMS,
                          population.begin(),
                          prg(0, N_DCCM));
        ...
        while (best < .999) {
                ...
                gpuErrchk(cudaPeekAtLastError());
                pickParents<<<TOTALTHREADS, BLOCKSIZE>>>(POPULATION_SIZE, N_ITEMS, randParents, scoresPtr, parentsPool_d);
                gpuErrchk(cudaPeekAtLastError());
                ...
                breedGeneration<<<TOTALTHREADS, BLOCKSIZE>>>(POPULATION_SIZE, N_ITEMS, randParams_d, popPtr, newPopPtr, parentsPool_d, mutPtr);
                gpuErrchk(cudaPeekAtLastError());
